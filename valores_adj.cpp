...
void imp_vec(const std::vector<double> &arr, std::ofstream &sal) {
...
std::vector<std::vector<double>> g6_mat(const std::string &s) {
...
int main() {
    for(int u = 9; u <= 10; ++u) {
        ...
        std::ofstream sal ("n" + std::to_string(u) + "/adj" + std::to_string(u) + ".csv");
        ...
        while(std::getline(adj, s)) {
            ...
            std::vector<std::vector<double>> mat_rep = g6_mat(s_rep);
