...
std::vector<std::vector<int>> toeplitz(const std::vector<std::bitset<10>> &m, const int &id) {
...
std::vector<int> berkowitz(const std::vector<std::bitset<10>> &a) {
...
void concurrente(int ini, int tam, const std::vector<std::vector<std::bitset<10>>> &mat, std::vector<par> &conten, std::mutex &mtx) {
    std::vector<par> aux;
    for(int i = 0; i < tam; ++i) {
        std::vector<int> poli = berkowitz(mat[ini + i]);
...
int main() {
    ...
    for(int u = 1; u <= 10; ++u) {
        ...
        if(u < 9) {
            for(int i = 0; i < mat.size(); ++i) {
                std::vector<int> poli = berkowitz(mat[i]);
        ...
        clock_t t1 = clock();
        std::ofstream sal("n" + std::to_string(u) + "/adj" + std::to_string(u) + ".dat");
        sal << conten[0].id;
